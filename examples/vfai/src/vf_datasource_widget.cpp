...
#include "vf_datasource_widget.hpp"
#include "vf_treeview.hpp"
#include "vfai/vf_config.hpp"


constexpr char vf::DataSourceWidget::HOSTNAME[];
constexpr char vf::DataSourceWidget::PORT[];


vf::DataSourceWidget::DataSourceWidget(QWidget *parent)
    : QWidget(parent)
{
  initUI();
  initConnections();
...
void vf::DataSourceWidget::initUI()
{
  auto source_type_lb = new QLabel("Data streamed from: ");
  source_type_cb_ = new QComboBox();
  source_type_cb_->addItem("Run directory", static_cast<int>(DataSourceType::file));
  source_type_cb_->addItem("ZeroMQ bridge", static_cast<int>(DataSourceType::zmq));
...
vf::DataSourceType vf::DataSourceWidget::sourceType() const
{
  return static_cast<DataSourceType>(source_type_cb_->itemData(source_type_cb_->currentIndex()).toInt());
}

void vf::DataSourceWidget::updateSourceList(const QStringList& srcs)
{
  list_model_->setSourceList(srcs);
}

void vf::DataSourceWidget::onStart()
{
...
void vf::DataSourceWidget::onStop()
{
